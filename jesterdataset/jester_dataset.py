# -*- coding: utf-8 -*-
# @Time    : 2020/7/19 10:45 PM
# @Author  : Yinghao Qin
# @Email   : y.qin@hss18.qmul.ac.uk
# @File    : jester_dataset.py
# @Software: PyCharm
...
class JesterDataset(Dataset):
    """
    This class is used to load the 20BN-JESTER dataset.
    """
    ...
    def __init__(self, csv_file="./jester_data/annotation_train.csv", video_dir="./jester_data/20bn-jester-v1",
                 frame_file_ending="jpg", number_of_frames=8, frame_select_strategy=FrameSelectStrategy.RANDOM,
                 frame_padding=FramePadding.REPEAT_END, video_transform=None):
        """
        A pytorch dataset to load the 20BN-JESTER dataset.
        :param csv_file: Path to the csv file, describing the videos. In jester this is eg. "1;Swipe Right" where 1 is
                the video folder name.
        :param video_dir: Path to the directory containing the videos.
        :param frame_file_ending: File ending of the video images, eg. "jpg" for file names like "0001.jpg"
        :param number_of_frames: The number of frames to get from one video.
        :param frame_select_strategy: When the video has more frames than number_of_frames, then the frame_select_strategy
                is used to select a subset of the frames.
        :param frame_padding: If the video does not have enough frames, the frame padding strategy is used to fill the frames.
        :param video_transform: Videotransforms to apply to the frames. You can not use the usual torchvision.transforms
